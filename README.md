...
기본 SPI RTL, Master/Slave UVM testbench, AXI4-Lite 기반 SPI Master 및 MicroBlaze용 bare-metal C application으로 구성함.
...
- MicroBlaze 기반 bare-metal C application
...
SPI Master, SPI Slave 및 AXI4-Lite SPI Master를 각각 독립된 UVM testbench에서 검증함. Synopsys VCS에서 테스트를 실행하고 FSDB 파형을 생성하여 Verdi에서 동작과 Coverage를 확인함. Monitor에서 SPI 신호와 수신 데이터를 수집하고, Scoreboard에서 예상 데이터와 DUT 출력값을 비교함.
...
| --- | --- | --- |
| Random Smoke Test | 2,000 | 랜덤 `tx_data`, SPI Mode 0~3 및 SPI 클럭 조합 |
| Pattern Test | 1,000 | `0x00`, `0xFF`, `0xAA`, `0x55` 반복 패턴과 랜덤 SPI Mode 및 SPI 클럭 조합 |
...
| Test | Transaction | 검증 내용 |
| --- | --- | --- |
| Random Smoke Test | 1,000 | 랜덤 `tx_data`와 SCLK Half-Period 조합 |
| Pattern Test | 1,000 | `0x00`, `0xFF`, `0xAA`, `0x55` 반복 패턴과 랜덤 SCLK Half-Period 조합 |
...
SPI Clock, TX Data 및 Control 레지스터 Write와 Status/RX 레지스터 Read 동작을 확인함. 선택된 Chip Select, MOSI 전송 데이터, `STATUS[15:8]` RX Data 및 `STATUS[0]`의 `busy` 상태를 예상값과 비교함.
...
| --- | --- | --- | --- |
| SPI Master | Random Smoke Test | Pass | ≈ 96% |
| SPI Master | Pattern Test | Pass | ≈ 89% |
| SPI Slave | Random Smoke Test | Pass | ≈ 90% |
| SPI Slave | Pattern Test | Pass | ≈ 81% |
| AXI4-Lite SPI Master | Random AXI/SPI Test | Pass | 100% |

각 Coverage는 테스트별 독립 실행 결과이며, AXI4-Lite SPI Master는 100% Coverage를 달성함.
...
MicroBlaze CPU에서 Vitis bare-metal application을 실행함. UART terminal에서 명령과 송신 문자를 입력하고 AXI4-Lite SPI register를 제어함.
